...
export default function ZoomTunnel() {
    ...
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          start: "top top", // Pins immediately when it hits the viewport
          end: "+=300%",    // Adjust to control the total scroll distance of the zoom
          pin: true,
          scrub: true,
      ...
      // 1. Fade in the white text mask while it stays perfectly still
      tl.fromTo(
        containerRef.current,
        { autoAlpha: 0 },
        { autoAlpha: 1, duration: 1 }
      )
      // 2. Fade in the psychedelic background & internal text
      .fromTo(
        [bgSvgRef.current, contentRef.current],
        { autoAlpha: 0 },
        { autoAlpha: 1, duration: 1 }
      )
      // 3. Zoom the SVG path mask
      // 3. Zoom the SVG path mask
      // 3. Zoom the SVG path mask
      .fromTo(
        pathRef.current,
        {
          // Dynamically scale down on mobile, keeping 80px padding
        ...
        {
          scale: 800, // MASSIVE scale to guarantee it clears wide laptop/desktop monitors
          x: () => (window.innerWidth - 489) / 2, // THIS MUST BE 489 so the camera doesn't slide sideways!
          y: () => (window.innerHeight - 72) / 2,
          transformOrigin: "44% 50%", 
          duration: 3, 
          ease: "power2.inOut"
  ...
  />
      ...
      {/* Make sure bg-transparent is here so the 3D fluid canvas is visible behind it! */}
      <section ref={sectionRef} className="w-full h-screen min-h-[100svh] flex items-center justify-center relative overflow-hidden z-10 bg-transparent">
        ...
        {/* The Masked Container - Fades in from autoAlpha 0 via GSAP */}
        <div 
          ref={containerRef}
          ...
          >
            <defs>
              <radialGradient id="swirlGradient" cx="50%" cy="50%" r="80%">
                <stop offset="0%" stopColor="#ff7eb3" />   {/* Bubblegum Pink */}
                <stop offset="50%" stopColor="#7ec8ff" />  {/* Cotton Candy Blue */}
                <stop offset="100%" stopColor="#e8a2ff" /> {/* Soft Lavender/Pink */}
              </radialGradient>
      ...
      {/* The White Void - This naturally scrolls up AFTER the zoom finishes */}
      <section className="w-full min-h-[100svh] bg-white flex flex-col justify-center items-center text-[#020202] relative z-20 py-24 px-6 sm:px-12 lg:px-24">
        <div className="max-w-4xl w-full flex flex-col gap-8">
          <p className="font-mono text-[10px] text-neutral-500 uppercase tracking-[0.3em]">
            [ THE_OTHER_SIDE // INITIALIZED ]
          </p>
          <h2 className="text-[clamp(2rem,6vw,5rem)] font-black uppercase tracking-tighter leading-[0.85]">
            Welcome to the Void.
          </h2>
          <div className="w-12 h-[2px] bg-black/20" />
          <p className="text-base sm:text-lg md:text-xl font-medium leading-relaxed tracking-tight text-neutral-700">
            You've successfully scrubbed through the vector tunnel. This section is naturally placed immediately after the pinned GSAP animation in the DOM hierarchy. 
            <br /><br />
            Because we rely on native scrolling and standard Tailwind structures, you can build out any standard content, paragraphs, or brutalist layouts directly below this. The GSAP context automatically cleans up after itself to keep your Next.js router from leaking memory.
          </p>
        </div>
      </section>
    </div>
